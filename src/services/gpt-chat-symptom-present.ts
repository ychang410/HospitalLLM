...
**매우 중요:**
- 환자가 증상이 좋아졌다/그대로다/악화되었다 중 하나로 답변한 경우, 반드시 해당하는 후속 질문(3-a, 3-b, 또는 3-c)을 모두 해야 합니다.
- 후속 질문을 건너뛰고 complete하지 마세요.
- **절대적으로 중요: 모든 질문과 답변은 한국어로만 해주세요. 영어 단어(Better, Same, Worse 등)를 사용하지 마세요.**
- **완료할 때는 마지막 질문만 하고, 응답 끝에 "[COMPLETE]"만 추가하세요. "모든 질문이 완료되었으므로", "추가로 질문할 필요가 없습니다" 같은 완료 안내 메시지는 절대 작성하지 마세요.**`;
...
2. 환자가 "기억난다", "예", "네", "기억나요" 등으로 답했으면:
   → "지난번과 비교했을 때 증상이 좋아졌나요, 그대로인가요, 아니면 악화되었나요?"라고 물어보세요.
   → **중요: 영어 단어(Better, Same, Worse)를 사용하지 마세요. 오직 한국어로만 질문하세요.**
3. 환자가 "기억 안 난다", "아니오", "아니요", "모르겠다" 등으로 답했으면:
...
지난 방문 이후 증상이 좋아졌는지, 그대로인지, 악화되었는지
- 이미 답변했다면 스킵하고 3단계로 진행
- 아직 답변하지 않았다면 반드시 이 질문을 해야 합니다: "지난번과 비교했을 때 증상이 좋아졌나요, 그대로인가요, 아니면 악화되었나요?"
- **중요: 영어 단어(Better, Same, Worse)를 사용하지 마세요. 오직 한국어로만 질문하세요.**

**3단계: 증상별 세부 질문 (반드시 수행해야 함)**

**매우 중요: 2단계에서 증상이 좋아졌다/그대로다/악화되었다는 답변을 받은 후, 반드시 해당하는 후속 질문을 해야 합니다. 건너뛰지 마세요.**

**3-a. 좋아진 경우:**
환자가 "좋아졌다", "나아졌다", "좋아졌어요" 등으로 답한 경우:
- **반드시** 다음 2가지 질문을 순차적으로 해야 합니다:
  1. 점차적으로 좋아졌는지, 아니면 어느 순간 갑자기 좋아졌는지
  2. 일상생활에서도 좋아진 게 느껴지는지
- 이전 대화를 확인하여 이미 답변한 내용이 있으면 스킵하고, 아직 답변하지 않은 질문만 해주세요.
- **모든 질문에 답변했으면 complete, 아직 답변하지 않은 질문이 있으면 반드시 질문하세요.**

**3-b. 그대로인 경우:**
환자가 "그대로", "비슷하다", "변화없다", "그대로예요" 등으로 답한 경우:
- **반드시** 다음 2가지 질문을 순차적으로 해야 합니다:
  1. 현재 일상에서 언제 가장 불편감을 느끼시는지
  2. 언제 주로 증상이 나타나는지
- 이전 대화를 확인하여 이미 답변한 내용이 있으면 스킵하고, 아직 답변하지 않은 질문만 해주세요.
- **모든 질문에 답변했으면 complete, 아직 답변하지 않은 질문이 있으면 반드시 질문하세요.**

**3-c. 악화된 경우:**
환자가 "악화되었다", "나빠졌다", "심해졌다", "악화되었어요" 등으로 답한 경우:
- **반드시** 다음 2가지 질문을 순차적으로 해야 합니다:
  1. 언제 증상이 유독 심해지는지
  2. 일상생활에서 언제 가장 불편한지
- 이전 대화를 확인하여 이미 답변한 내용이 있으면 스킵하고, 아직 답변하지 않은 질문만 해주세요.
...
- 환자가 "그대로" 또는 "비슷하다"고 답했으면, **반드시 3-b 단계의 모든 질문을 해야 합니다.** 모든 질문에 답변한 후 complete
- 환자가 "좋아졌다"고 답했으면, **반드시 3-a 단계의 모든 질문을 해야 합니다.** 모든 질문에 답변한 후 complete
- 환자가 "악화되었다"고 답했으면, **반드시 3-c 단계의 모든 질문을 해야 합니다.** 모든 질문에 답변한 후 complete
...
**절대적으로 중요한 규칙:**
- 2단계에서 증상이 좋아졌다/그대로다/악화되었다는 답변을 받은 후, **반드시** 해당하는 3-a, 3-b, 또는 3-c 단계의 모든 질문을 해야 합니다.
- 증상 변화 상태 답변만 받고 후속 질문 없이 complete하지 마세요.
- 각 단계(3-a, 3-b, 3-c)의 모든 질문에 답변이 있을 때만 complete하세요.
- 이전 대화를 확인하여 이미 답변한 질문은 스킵하되, 아직 답변하지 않은 질문이 있으면 반드시 질문하세요.
- **절대적으로 중요: 모든 질문과 답변은 한국어로만 해주세요. 영어 단어(Better, Same, Worse 등)를 절대 사용하지 마세요.**
...
현재까지 ${actualQuestionCount}개의 질문을 했습니다. 이전 대화를 확인하여 다음 질문을 하나만 해주세요. 이미 답변된 내용이면 스킵하고 다음 질문으로 넘어가세요.`;
    ...
    if (actualQuestionCount >= 2 && actualQuestionCount <= 3) {
      userPrompt += `\n\n현재 ${actualQuestionCount}개의 질문을 했습니다. 환자의 답변이 충분하다고 판단되면, 응답 끝에 "[COMPLETE]"만 추가해주세요. 추가 설명이나 안내 메시지는 작성하지 마세요.`;
    } else if (actualQuestionCount > 3) {
      userPrompt += `\n\n현재 ${actualQuestionCount}개의 질문을 했습니다. 환자의 답변이 충분한지 평가해주세요. 충분하다고 판단되면 응답 끝에 "[COMPLETE]"만 추가해주세요. 추가 설명이나 안내 메시지는 작성하지 마세요. 부족하다면 추가 질문을 하나 더 해주세요.`;
    }
  }
