...
import {
  chatAboutSymptom,
  chatAboutExamination,
  SymptomChatResponse,
  getMainDiagnosisIntroMessages,
  getSymptomIntroMessage,
  getSymptomInitialQuestion,
  getExaminationMessages,
  getExaminationEmptyMessage,
  getMainDiagnosisCompletionMessage,
} from "../services/gpt-chat-main-diagnosis";
import {
  generateNewPainQuestion,
  getOtherNewPainIntroMessages,
  getOtherPainEmptyMessage,
  getOtherPainFirstQuestion,
  getNewPainHardcodedMessage,
  getNewPainCompletionMessage,
  chatAboutNewPain,
  NewPainChatResponse,
} from "../services/gpt-chat-new-pain";
import {
  generateSideEffectQuestion,
  chatAboutSideEffects,
  SideEffectChatResponse,
  getSideEffectsIntroMessage,
  getMedicationHardcodedMessage,
  getMedicationEmptyMessage,
  getSideEffectCompletionMessage,
} from "../services/gpt-chat-side-effects";
import {
  generateAdditionalQuestion,
  chatAboutAdditional,
  AdditionalChatResponse,
  getAdditionalIntroMessage,
  getAdditionalHardcodedMessage,
  getAdditionalCompletionMessage,
} from "../services/gpt-chat-additional";
import {
  determineBodyPartForSymptom,
  extractSymptomsFromMessage,
} from "../services/gpt-body-part";
...
export type MainDiagnosisSubSection =
  | "diagnosis_a"
  | "diagnosis_b"
  | "diagnosis_c"
  | "examination";
...
export const sideEffectSubSections: {
  key: SideEffectSubSection;
  label: string;
}[] = [{ key: "medication", label: "복용 약" }];

export const additionalSubSections: {
  key: AdditionalSubSection;
  label: string;
...
export default function ChatInterface({
  ...
  const [currentCategory, setCurrentCategory] =
    useState<Category>("main_diagnosis"); // 초기에는 주요 진단 내용 선택
  const [maxReachedIndex, setMaxReachedIndex] = useState<number>(0); // 초기에는 주요 진단 내용까지 도달
  const [currentMainDiagnosisSubSection, setCurrentMainDiagnosisSubSection] =
    useState<MainDiagnosisSubSection | null>(null);
  const [
    completedMainDiagnosisSubSections,
    setCompletedMainDiagnosisSubSections,
  ] = useState<Set<MainDiagnosisSubSection>>(new Set());
  const [
    maxReachedMainDiagnosisSubSectionIndex,
    setMaxReachedMainDiagnosisSubSectionIndex,
  ] = useState<number>(-1);
  const [currentNewPainSubSection, setCurrentNewPainSubSection] =
    useState<NewPainSubSection | null>(null);
  const [completedNewPainSubSections, setCompletedNewPainSubSections] =
    useState<Set<NewPainSubSection>>(new Set());
  const [
    maxReachedNewPainSubSectionIndex,
    setMaxReachedNewPainSubSectionIndex,
  ] = useState<number>(-1);
  const [currentSideEffectSubSection, setCurrentSideEffectSubSection] =
    useState<SideEffectSubSection | null>(null);
  const [completedSideEffectSubSections, setCompletedSideEffectSubSections] =
    useState<Set<SideEffectSubSection>>(new Set());
  const [
    maxReachedSideEffectSubSectionIndex,
    setMaxReachedSideEffectSubSectionIndex,
  ] = useState<number>(-1);
  const [currentAdditionalSubSection, setCurrentAdditionalSubSection] =
    useState<AdditionalSubSection | null>(null);
  const [completedAdditionalSubSections, setCompletedAdditionalSubSections] =
    useState<Set<AdditionalSubSection>>(new Set());
  const [
    maxReachedAdditionalSubSectionIndex,
    setMaxReachedAdditionalSubSectionIndex,
  ] = useState<number>(-1);

  const patientDisplayName = patientInfo?.name
    ? `${patientInfo.name}님`
    : "환자님";
  ...
  const mainDiagnosisSubSections = useMemo(() => {
    ...
    if (
      medicalRecordAnalysis?.symptoms &&
      medicalRecordAnalysis.symptoms.length >= 3
    ) {
      sections.push(
  ...
  const [loadedSubSections, setLoadedSubSections] = useState<Set<string>>(
    new Set()
  );
  const mainDiagnosisIntroShownRef = useRef(false);
  const newPainIntroShownRef = useRef(false);
  const newPainQuestionTimeoutRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null); // GPT 질문 생성 setTimeout 추적
  const [isGeneratingIntro, setIsGeneratingIntro] = useState(false);
  const [isProcessingGPT, setIsProcessingGPT] = useState(false);
  const [completedSymptomSubSections, setCompletedSymptomSubSections] =
    useState<Set<string>>(new Set()); // 완료된 증상 서브섹션 추적
  const [highlightedBodyParts, setHighlightedBodyParts] = useState<BodyPart[]>(
    []
  ); // 하이라이트된 신체 부위
  ...
  const getStorageKey = () => {
    if (
      ...
      const birthDate = `${
        patientInfo.birthYear
      }-${patientInfo.birthMonth.padStart(
        2,
        "0"
      )}-${patientInfo.birthDay.padStart(2, "0")}`;
  ...
  const removePatientNameFromContent = (content: string): string => {
    ...
    let cleanedContent = content.replace(
      new RegExp(nameWithHonorific, "g"),
      "환자님"
    );
  ...
  const saveConversationToLocalStorage = () => {
    try {
      ...
      messagesBySection.forEach((messages, sectionKey) => {
        ...
        const subSection =
          subSectionParts.length > 0 ? subSectionParts.join("_") : undefined;
  ...
  const loadConversationFromLocalStorage = (): boolean => {
    try {
      ...
      Object.entries(conversationLog.conversations).forEach(
        ([sectionKey, data]) => {
          restoredMessages.set(
            sectionKey,
  ...
  const downloadConversationLog = () => {
    try {
      ...
      if (
        ...
        filename = `conversationLogs/conversation_log_${birthDate}_${
          new Date().toISOString().split("T")[0]
        }.json`;
      } else {
        filename = `conversationLogs/conversation_log_age${patientInfo.age}_${
          new Date().toISOString().split("T")[0]
        }.json`;
      }
  ...
  const handleCategoryChange = (category: Category) => {
    ...
    if (category === "other_new_pain") {
      ...
      if (
        ...
        completedMainDiagnosisSubSections.size < mainDiagnosisSubSections.length
      ) {
        return;
      }
      setCurrentCategory(category);
    ...
    if (
      !TEST_MODE &&
      categoryIndex === 2 &&
      completedNewPainSubSections.size < newPainSubSections.length
    ) {
      return;
    }
    ...
    if (
      !TEST_MODE &&
      categoryIndex === 3 &&
      completedSideEffectSubSections.size < sideEffectSubSections.length
    ) {
      return;
    }
    ...
    if (category === "side_effects") {
      ...
      if (
        !existingIntroMessages ||
        existingIntroMessages.length === 0 ||
        isSameCategory
      ) {
  ...
  const handleMainDiagnosisSubSectionChange = async (
    subSection: MainDiagnosisSubSection
  ) => {
    const subSectionIndex = mainDiagnosisSubSections.findIndex(
      (s) => s.key === subSection
    );
    ...
    if (
      subSection === "diagnosis_a" ||
      subSection === "diagnosis_b" ||
      subSection === "diagnosis_c"
    ) {
      // 증상 정보 가져오기
      const symptomIndex =
        subSection === "diagnosis_a" ? 0 : subSection === "diagnosis_b" ? 1 : 2;
    ...
    if (!isActivated) {
      ...
      if (
        ...
        const symptomName =
          symptom?.name ||
          mainDiagnosisSubSections.find((s) => s.key === subSection)?.label ||
          "증상";
        const mentioned = symptom?.mentioned ?? false;
        const present = symptom?.present ?? false;

        // 첫 번째 하드코딩된 메시지: 증상 소개
        const introMessage = getSymptomIntroMessage(
          subSectionIndex,
          symptomName
        );

        // 두 번째 하드코딩된 메시지: 증상 관련 질문
        const initialMessage = getSymptomInitialQuestion(
          mentioned,
          present,
          symptomName
        );
            ...
            setCompletedMainDiagnosisSubSections((prev) => {
              ...
              if (nextSubSectionIndex < mainDiagnosisSubSections.length) {
                const nextSubSection =
                  mainDiagnosisSubSections[nextSubSectionIndex];

                // 다음 서브섹션이 diagnosis_a, b, c, examination 중 하나인 경우 자동 이동
                if (
                  nextSubSection.key === "diagnosis_a" ||
                  nextSubSection.key === "diagnosis_b" ||
                  nextSubSection.key === "diagnosis_c" ||
                  nextSubSection.key === "examination"
                ) {
                  setTimeout(() => {
                ...
                if (updated.size === mainDiagnosisSubSections.length) {
                  ...
                  const currentCategoryIndex =
                    categoryOrder.indexOf("main_diagnosis");
  ...
  const handleSymptomComplete = async (
    subSection: MainDiagnosisSubSection,
  ...
  const handleNewPainSubSectionChange = async (
    subSection: NewPainSubSection
  ) => {
    const subSectionIndex = newPainSubSections.findIndex(
      (s) => s.key === subSection
    );
    if (subSectionIndex === -1) return;
    ...
    if (subSection === "other_pain") {
      ...
      if (!isActivated) {
        ...
        setMessagesBySection((prev) => {
          ...
          const existingMessages = newMap.get(sectionKey) || [];
          const hasHardcodedMessage = existingMessages.some(
            (msg) => msg.content === getNewPainHardcodedMessage()
          );
          ...
          try {
            ...
            const timeoutId = setTimeout(() => {
              setMessagesBySection((prev) => {
                ...
                const hasGptQuestion = existingMessages.some(
                  (msg) =>
                    msg.role === "assistant" &&
  ...
  const handleSideEffectSubSectionChange = async (
    subSection: SideEffectSubSection
  ) => {
    const subSectionIndex = sideEffectSubSections.findIndex(
      (s) => s.key === subSection
    );
    if (subSectionIndex === -1) return;
        ...
        if (medicationNames.length === 0) {
          ...
          setTimeout(() => {
            ...
            if (sideEffectSubSections.length === 1) {
              ...
              // 다음 카테고리로 자동 전환 (2.5초 후)
              const currentCategoryIndex =
                categoryOrder.indexOf("side_effects");
              if (currentCategoryIndex < categoryOrder.length - 1) {
                const nextCategory = categoryOrder[currentCategoryIndex + 1];
                setTimeout(() => {
                  handleCategoryChange(nextCategory);
          ...
          const medicationsText =
            medicationNames.length === 1
              ...
              : medicationNames.slice(0, -1).join(", ") +
                ", " +
                medicationNames[medicationNames.length - 1];
    ...
    if (
      TEST_MODE ||
      subSectionIndex === maxReachedSideEffectSubSectionIndex + 1
    ) {
      setMaxReachedSideEffectSubSectionIndex(subSectionIndex);
    }
  };

  const handleAdditionalSubSectionChange = async (
    subSection: AdditionalSubSection
  ) => {
    const subSectionIndex = additionalSubSections.findIndex(
      (s) => s.key === subSection
    );
    if (subSectionIndex === -1) return;
    ...
    if (
      TEST_MODE ||
      subSectionIndex === maxReachedAdditionalSubSectionIndex + 1
    ) {
      ...
      introMessages.forEach((content, index) => {
        setTimeout(() => {
          ...
          setMessagesBySection((prev) => {
            ...
            newMap.set("main_diagnosis_intro", [
              ...existingMessages,
              newMessage,
            ]);
            return newMap;
      ...
      introMessages.forEach((content, index) => {
        setTimeout(() => {
          ...
          setMessagesBySection((prev) => {
            ...
            newMap.set("other_new_pain_intro", [
              ...existingMessages,
              newMessage,
            ]);
            return newMap;
  ...
  useEffect(() => {
    const loadScript = (category: Category, subSectionKey: string) => {
      ...
      const scriptMessages: Message[] = script.map(
  ...
  const handleForceComplete = async (
    ...
    if (category === "main_diagnosis" && subSection) {
      ...
      // 완료 상태 설정 (두 가지 모두 설정)
      setCompletedSymptomSubSections(
        (prev) => new Set([...prev, subSectionKey])
      );
      setCompletedMainDiagnosisSubSections((prev) => {
        const updated = new Set(prev);
      ...
      if (subSection === "other_pain") {
          ...
          try {
            ...
            if (response.isComplete) {
              ...
              const isCompletionMessage =
                assistantMessage.content.includes("답변 감사합니다");
              ...
              if (isCompletionMessage) {
                setCompletedNewPainSubSections((prev) => {
                  ...
                  if (subSectionIndex + 1 < newPainSubSections.length) {
                    const nextSubSection =
                      newPainSubSections[subSectionIndex + 1];
                    setTimeout(() => {
                      handleNewPainSubSectionChange(nextSubSection.key);
                    }, 2500);
                  } else {
                    ...
                    if (updated.size === newPainSubSections.length) {
                      ...
                      // 다음 카테고리로 자동 전환 (2.5초 후)
                      const currentCategoryIndex =
                        categoryOrder.indexOf("other_new_pain");
                      if (currentCategoryIndex < categoryOrder.length - 1) {
                        const nextCategory =
                          categoryOrder[currentCategoryIndex + 1];
                        setTimeout(() => {
        ...
        if (userMessages.length === 1 && medicalRecordAnalysis) {
          ...
          try {
            ...
            const extractedSymptoms = await extractSymptomsFromMessage(
              firstUserMessage
            );
            ...
            if (extractedSymptoms.length > 0) {
              ...
              // null이 아닌 바디 파트만 필터링
              const validBodyParts = bodyParts.filter(
                (bp): bp is BodyPart => bp !== null
              );
        ...
        const conversationUserMessages = conversationHistory.filter(
          (msg) => msg.role === "user"
        );
        ...
        try {
          ...
          if (response.isComplete) {
            ...
            setCompletedAdditionalSubSections((prev) => {
              ...
              if (
                updated.size === additionalSubSections.length &&
                onConversationComplete
              ) {
  ...
  return (
    <div className="w-[1366px] h-[1024px] relative bg-white overflow-hidden">
      ...
      <ChatHeader
        ...
        isMainDiagnosisComplete={
          TEST_MODE ||
          completedMainDiagnosisSubSections.size ===
            mainDiagnosisSubSections.length
        }
        isNewPainComplete={
          TEST_MODE ||
          completedNewPainSubSections.size === newPainSubSections.length
          ...
          <div className="bg-white rounded-xl shadow-lg border border-zinc-300 px-4 py-3">
            ...
            <div className="flex gap-3">
              {newPainSubSections.map((subSection, index) => {
                ...
                const isCompleted = completedNewPainSubSections.has(
                  subSection.key
                );
                ...
                const isReachable =
                  TEST_MODE ||
                  index === 0 ||
                  index <= maxReachedNewPainSubSectionIndex + 1;
          ...
          <div className="bg-white rounded-xl shadow-lg border border-zinc-300 px-4 py-3">
            <div className="flex gap-3">
              {sideEffectSubSections.map((subSection, index) => {
                ...
                const isCompleted = completedSideEffectSubSections.has(
                  subSection.key
                );
                // 테스트 모드에서는 모두 클릭 가능
                const isReachable =
                  TEST_MODE ||
                  (maxReachedSideEffectSubSectionIndex === -1
                    ? index === 0
          ...
          <div className="bg-white rounded-xl shadow-lg border border-zinc-300 px-4 py-3">
            <div className="flex gap-3">
              {additionalSubSections.map((subSection, index) => {
                ...
                const isCompleted = completedAdditionalSubSections.has(
                  subSection.key
                );
                // 테스트 모드에서는 모두 클릭 가능
                const isReachable =
                  TEST_MODE ||
                  (maxReachedAdditionalSubSectionIndex === -1
                    ? index === 0
          ...
          <div className="bg-white rounded-xl shadow-lg border border-zinc-300 px-4 py-3">
            ...
            <div className="flex gap-3">
              {mainDiagnosisSubSections.map((subSection, index) => {
                const isActive =
                  currentMainDiagnosisSubSection === subSection.key;
                const isCompleted = completedMainDiagnosisSubSections.has(
                  subSection.key
                );
                // 첫 번째 서브 섹션만 클릭 가능, 나머지는 순서대로 (테스트 모드에서는 모두 클릭 가능)
                const isReachable =
                  TEST_MODE ||
                  (maxReachedMainDiagnosisSubSectionIndex === -1
                    ? index === 0 // 처음에는 첫 번째만
                ...
                <div className="self-stretch justify-start text-black text-[26px] font-normal font-inter">
                  {currentCategory === null
                    ...
                    ? `${categoryLabels[currentCategory]} - ${
                        mainDiagnosisSubSections.find(
                          (s) => s.key === currentMainDiagnosisSubSection
                        )?.label
                      }(를) 작업중입니다 ...`
                    : currentCategory === "main_diagnosis"
                    ...
                    : currentCategory === "other_new_pain" &&
                      currentNewPainSubSection
                    ? `${categoryLabels[currentCategory]} - ${
                        newPainSubSections.find(
                          (s) => s.key === currentNewPainSubSection
                        )?.label
                      }을(를) 작업중입니다 ...`
                    ...
                    : currentCategory === "side_effects" &&
                      currentSideEffectSubSection
                    ? `${categoryLabels[currentCategory]} - ${
                        sideEffectSubSections.find(
                          (s) => s.key === currentSideEffectSubSection
                        )?.label
                      }을(를) 작업중입니다 ...`
                    : currentCategory === "side_effects"
                    ...
                    : currentCategory === "additional_questions" &&
                      currentAdditionalSubSection
                    ? `${categoryLabels[currentCategory]} - ${
                        additionalSubSections.find(
                          (s) => s.key === currentAdditionalSubSection
                        )?.label
                      }을(를) 작업중입니다 ...`
                    : currentCategory === "additional_questions"
                    ? `${categoryLabels[currentCategory]}을(를) 작업중입니다 ...`
        ...
        <div className="p-5 border-t border-gray-300">
          <ChatInput
            ...
            disabled={
              ...
              (currentCategory === "main_diagnosis" &&
                currentMainDiagnosisSubSection &&
                (currentMainDiagnosisSubSection === "diagnosis_a" ||
                  currentMainDiagnosisSubSection === "diagnosis_b" ||
                  currentMainDiagnosisSubSection === "diagnosis_c") &&
                completedSymptomSubSections.has(
                  `main_diagnosis_${currentMainDiagnosisSubSection}`
                )) ||
              isProcessingGPT ||
              (currentMessages.length > 0 &&
                currentMessages[currentMessages.length - 1].content.includes(
                  "답변 감사합니다"
                ))
            }
          />
